...
* 上記の入力が完了したら、次にメニューバーを見ると、「TBOTControl」というものが表示されており、こちらを押すと、
  ...
  * Test order
* という3つの選択肢があります.
* こちらはbotの稼働やテストをしてくれるもので、一度Test orderをすると入力項目のチェックとメッセージがちゃんと送れるかどうか、メールが来ていたらちゃんとオーダーを送ることができるかをチェックすることができます。
* では1回Test orderをしましょう
  * このとき、Tradingviewからアラートメッセージが来ていて未読な場合、エラーが発生しますので、一旦それを既読にするか削除にしておきましょう。
  * ちゃんと入力されていれば「テストメッセージ」という文面が通知媒体に流れるて、エラーなく終わるはずです。エラーが出た場合はメッセージに従って入力項目を修正してください。
...
* 次にアラートメッセージを設定します。
  ...
  * 実際の設定だと画像のとおりになります。<img src="images/alertmessage.png" width=480>
* とりあえず一回送ってみましょう.
